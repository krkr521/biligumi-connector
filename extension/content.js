...
(() => {
  ...
  let tokenStorageChangeHandler = null;
  ...
  function bootstrapConnector() {
    ...
    preloadChromeStorage(storageKeys, (ready) => {
      ...
      refreshBridgedInitialStateWithRetry().finally(runUserscriptConnector);
    });
  }
...
(function () {
  ...
  function getPageInitialState() {
    return getBridgedInitialState();
  }

  function refreshPageInitialState() {
    return refreshBridgedInitialStateWithRetry();
  }

  function invalidatePageInitialState() {
    invalidateBridgedInitialState();
  }
  ...
  async function bindSubjectFromDirectInput(subjectId, isCurrentRequest = () => true) {
    ...
    try {
      ...
      if (!isRouteContextCurrent(routeContext) || !isCurrentRequest()) return;
      throw error;
    }
    ...
    state.error = "";
    await requestBindSubject(subjectId, routeContext, isCurrentRequest);
  }
  ...
  async function requestBindSubject(subjectId, routeContext = captureRouteContext(), isCurrentRequest = () => true) {
    ...
    if (!Number.isFinite(safeSubjectId) || safeSubjectId <= 0 || !isRouteContextCurrent(routeContext) || !isCurrentRequest()) return;
    ...
    if (readiness.action === "wait") {
      beginLongVideoBindingWait(safeSubjectId, routeContext, { isCurrentRequest });
      return;
    }
    if (readiness.action === "auto") {
      await setLongVideoEpisodeModeDecision(true).catch(reportError);
      if (!isRouteContextCurrent(routeContext) || !isCurrentRequest()) return;
      await bindSubject(safeSubjectId, routeContext, isCurrentRequest).catch(reportError);
      return;
    }
    await bindSubject(safeSubjectId, routeContext, isCurrentRequest).catch(reportError);
  }
  ...
  function retryLongVideoBindingWait() {
    ...
    if (!isRouteContextCurrent(pending.bindingContext) || (pending.isCurrentRequest && !pending.isCurrentRequest())) {
      clearLongVideoBindingPrompt({ render: true });
      return;
    }
  ...
  async function resolveLongVideoBindingPrompt(enabled) {
    ...
    const isCurrent = () => isRouteContextCurrent(pending.bindingContext) && isCurrentRequest();
    if (!isCurrent()) {
      clearLongVideoBindingPrompt({ render: true });
      return;
    }
  ...
  async function unbindSubject() {
    ...
    if (!ok || !isRouteContextCurrent(routeContext)) return;
    let applied = false;
    if (collectionRule) {
  ...
  function captureCollectionOperationContext() {
    return { subjectId: Number(state.subjectId), token: String(state.token || ""), pageContext: captureRouteContext() };
  }
  ...
  function isCollectionOperationContextCurrent(context) {
    return Boolean(context
      ...
      && isRouteContextCurrent(context.pageContext));
  }
  ...
  function bindAccessTokenChanges() {
    tokenStorageChangeHandler = syncAccessTokenFromStorage;
  }
  ...
  async function applySettingsFromDialog() {
    ...
    await Promise.all([
      ...(hasValidReplacementToken ? [writeValueAsync(STORAGE.token, nextToken)] : []),
  ...
  function queueClearSavedAccessToken() {
    ...
    settingsPersistQueue = settingsPersistQueue
      .then(() => (state.token ? requestInlineConfirm({ context: "settings", danger: true, message: "确定清除已保存的 Access Token 吗？\n清除后需要重新填写 Token 才能访问需要登录的 Bangumi 功能。" }) : false))
      .then((confirmed) => (confirmed && state.token === tokenSnapshot ? clearSavedAccessToken() : undefined))
      .catch(showError);
  ...
  function bgmRequestOnce(method, url, data, options) {
    ...
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        ...
        onerror: (response) => {
          const errorKind = String(response && response.errorKind || "network");
          if (errorKind === "validation") {
            reject(new Error(response && response.error || "扩展已拒绝不安全的 Bangumi API 请求"));
            return;
          }
          reject(makeNetworkError("Bangumi API 请求失败，可能是网络或扩展跨域权限问题", "connection"));
        },
  ...
  async function updateStoredCollectionSegmentProgress(update) {
    return withBindingsLock(async () => {
      const stored = await readJsonValueFresh(STORAGE.collectionSegmentProgress, {});
      const next = normalizeCollectionSegmentProgress(stored);
      const normalizedChanged = JSON.stringify(next) !== JSON.stringify(stored);
      const changed = update(next) !== false;
      if (changed || normalizedChanged) await writeJsonValueAsync(STORAGE.collectionSegmentProgress, next);
  ...
  function refreshBridgedInitialState() {
    if (location.hostname !== "www.bilibili.com" || !isExtensionRuntimeAvailable()) return Promise.resolve({});
    const requestHref = location.href;
    const requestSeq = ++bridgedInitialStateRequestSeq;
    return new Promise((resolve) => {
      let settled = false;
      let timeoutId = 0;
      const finish = (value) => {
      ...
      try {
        chrome.runtime.sendMessage({ type: PAGE_STATE_MESSAGE }, (result) => {
          if (chrome.runtime.lastError || !result || !result.ok) {
            finish({});
            return;
          }
          const compatible = toInitialStateCompat(result.state, requestHref);
          if (
            requestSeq !== bridgedInitialStateRequestSeq
            || location.href !== requestHref
            || !Object.keys(compatible).length
          ) {
            finish({});
            return;
          }
          bridgedInitialState = compatible;
          bridgedInitialStateHref = requestHref;
          finish(compatible);
        });
      } catch (_error) {
        finish({});
  ...
  function refreshBridgedInitialStateWithRetry(retryDelays = [0, 350, 900], maxWaitMs = 2500) {
    if (location.hostname !== "www.bilibili.com" || !isExtensionRuntimeAvailable()) return Promise.resolve({});
    const delays = Array.isArray(retryDelays)
      ? retryDelays.map(Number).filter((delay) => Number.isFinite(delay) && delay >= 0)
      : [0];
    return new Promise((resolve) => {
      let settled = false;
      const timerIds = [];
      ...
      const attempt = () => {
        refreshBridgedInitialState().then((value) => {
          if (value && Object.keys(value).length) finish(value);
        }, () => {});
      };
      delays.forEach((delay) => {
        if (delay === 0) {
          ...
          timerIds.push(window.setTimeout(attempt, delay));
        }
      });
      timerIds.push(window.setTimeout(
        () => finish(getBridgedInitialState()),
        Math.max(1, Number(maxWaitMs) || 2500),
      ));
    });
  }

  function invalidateBridgedInitialState() {
    bridgedInitialStateRequestSeq += 1;
    bridgedInitialState = {};
    bridgedInitialStateHref = "";
  }

  function getBridgedInitialState() {
    return bridgedInitialStateHref === location.href ? bridgedInitialState : {};
  }

  function toInitialStateCompat(value, expectedHref = location.href) {
    const snapshot = value && typeof value === "object" ? value : {};
    if (snapshot.schemaVersion !== 1 || snapshot.href !== expectedHref) return {};
    const identity = snapshot.identity && typeof snapshot.identity === "object" ? snapshot.identity : {};
    let expectedUrl;
    try {
      expectedUrl = new URL(expectedHref);
    } catch (_error) {
      return {};
    }
    if (expectedUrl.protocol !== "https:" || expectedUrl.hostname !== "www.bilibili.com") return {};
    const videoPath = expectedUrl.pathname.match(/^\/video\/(BV[a-z0-9]+)/i);
    const episodePath = expectedUrl.pathname.match(/^\/bangumi\/play\/ep(\d+)/i);
    const seasonPath = expectedUrl.pathname.match(/^\/bangumi\/play\/ss(\d+)/i);
    const mediaPath = expectedUrl.pathname.match(/^\/bangumi\/play\/md(\d+)/i);
    if (videoPath && String(identity.bvid || "").toUpperCase() !== videoPath[1].toUpperCase()) return {};
    if (episodePath && String(identity.episodeId || "") !== episodePath[1]) return {};
    if (seasonPath && String(identity.seasonId || "") !== seasonPath[1]) return {};
    if (mediaPath && String(identity.mediaId || "") !== mediaPath[1]) return {};
    const titles = snapshot.titles && typeof snapshot.titles === "object" ? snapshot.titles : {};
    ...
    const mediaInfo = {
      season_id: String(identity.seasonId || ""),
      media_id: String(identity.mediaId || ""),
      title: String(titles.mediaTitle || titles.seasonTitle || ""),
      name: String(titles.mediaTitle || ""),
      season_title: String(titles.seasonTitle || ""),
    };
    const epInfo = {
      id: String(identity.episodeId || ""),
      season_title: String(titles.seasonTitle || ""),
      title: String(titles.episodeTitle || ""),
      long_title: String(titles.episodeLongTitle || ""),
      share_copy: String(titles.shareCopy || ""),
    };
    const videoData = {
    ...
    return {
      season_id: mediaInfo.season_id,
      media_id: mediaInfo.media_id,
      season_title: mediaInfo.season_title,
      bvid: videoData.bvid,
  ...
  function bindStorageMirrorUpdates() {
    ...
    chrome.storage.onChanged.addListener((changes, areaName) => {
      ...
      Object.entries(changes).forEach(([key, change]) => {
        ...
        if (key === "biligumi.token" && tokenStorageChangeHandler) {
          tokenStorageChangeHandler(change.newValue);
        }
      });
    });
  }
  ...
  function GM_xmlhttpRequest(details) {
    ...
    chrome.runtime.sendMessage({ type: HTTP_REQUEST_MESSAGE, request }, (result) => {
      ...
      if (!result || !result.ok) {
        const payload = {
          error: result && result.error || "Extension HTTP request failed",
          errorKind: result && result.errorKind || "network",
        };
        if (payload.errorKind === "timeout") callHandler(details.ontimeout, payload);
        else callHandler(details.onerror, payload);
        return;
      }
  ...
  function fetchHttpRequest(request, details) {
    ...
    fetch(request.url, {
      ...
      redirect: request.url.startsWith("https://bgm.tv/") ? "follow" : "error",
