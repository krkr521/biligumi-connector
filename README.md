...
- 按总进度把前 N 集批量标为看过。
- 单集切换：看过 / 未收藏。
- 播放器进度达到标准线后自动把当前集标为看过；标准线默认 50%，可在设置里按当前 UP / 页面来源以 10% 步长调整。单次向前跳转超过 5 分钟或打开页面时已在标准线之后，不会自动标记，避免拖进度条和 B 站恢复上次播放位置误触发。
...
- 支持弹幕悬停 `+1` 和本地弹幕收藏（交互参考 [qianjiachun/douyuEx](https://github.com/qianjiachun/douyuEx)）；发送时复用 Bilibili 当前弹幕输入栏的发送位置、颜色等设置。
...
- 白名单支持 UP 主 UID/名称、BV 号、页面 key 或 URL 片段；齿轮按钮会同时配置 Access Token、白名单、轻量候选开关、角色 / CV 横栏、条目信息栏、官方番剧页布局兼容开关、自动标记已看的进度标准线、当前番剧的 OP/ED 跳过按钮和全局快捷键。
