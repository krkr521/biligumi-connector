...
不建议把 Bilibili 官方番剧源作为主要使用场景；脚本会始终启用右侧栏兼容，但官方页面结构变化更频繁，出问题时建议改用普通视频源。
...
- Bangumi API 始终优先使用官方 `https://api.bgm.tv`。只有官方请求经过重试后仍超时或无法连接，才会弹出中继选择；用户必须明确点击“重试官方 API”或某个红色危险中继按钮，中继不会自动启用、不会保存为默认项，且只重试当前失败的请求。
...
- 主面板「我的完成度」旁可按视频切换「自动 / 暂停」进度追踪：暂停后不自动推测/高亮当前集，也不按进度自动标记看过；手动点击集数不受影响。
- 集数格子按 Bangumi 首播日期（airdate）着色：未播出显示灰色、onair 窗口内（今天或明天播出，对齐 Bangumi 对深夜档的判定）显示绿色；已看过保持深蓝、已播出未看过保持淡蓝；airdate 缺失的集不误标，保持淡蓝。悬停集数会显示 Bangumi 风格信息卡（集数、标题、中文标题、首播日期、时长），底部提示左键标记/取消看过、右键打开本集讨论。
- 长视频多集合一识别：普通视频超过 2 小时会询问是否在一个视频内包含连续多集正片；选择“是”后，才会按 Bangumi 单集时长和当前 UP 专属的“首集开始时间”估算当前集，并使用单集局部进度自动标记看过。设置中可勾选「遇到长视频时自动识别」（默认关闭）：勾选后跳过确认，直接按多集合一处理。Bangumi 动画条目的 `platform` 为「剧场版」或 `Movie` 时按动画电影处理；平台字段缺失、为 `WEB` 或无法识别时，再以“正片恰好 1 集且该集超过 1 小时”兜底。动画电影不会触发长视频集数推断。
- 长视频分集推测提示条内可一键把当前播放进度设为「本视频首集起点」（优先于 UP 默认），也可清除后回落 UP 默认。
- 同一 BV 的分 P 标题若包含 `S1`、`S2` 等季度标记，会按分 P 分别绑定对应的 Bangumi 季度条目；`S2 13-15` 这类范围会优先只使用第 13–15 集建立时间线，范围超出当前条目章节时则安全降级为整季推测且不自动标记。
- 普通投稿的多季度合集若使用 `1.1 / 1.2`、`第1集上 / 下`、`第二季0`、`S2E13` 等分 P 标题，会按“季度 + 集数范围”绑定不同 Bangumi 条目。只有 Bangumi 明确提供 `total_episodes`，且当前季度已覆盖该总集数时才会进入范围确认；总集数缺失或当前仍在更新时不触发分批检测。普通连续 `1…N` 可继续使用整 BV 绑定；未完结的分段、季度、EP0 等结构仍保持隔离，不继承整 BV 的旧绑定。每集拆成上下段时，必须两段都达到观看阈值才自动标记该集。
- 合集来源从第0集开始时，默认把来源第0集视为 Bangumi 正片列表的第一集，后续顺延；如果 Bangumi API 的 `type=0` 正片明确包含 `sort=0`，则保留 EP0 标签并从该条开始对齐。
- Bilibili 官方番剧页会用右侧 `(当前项/总数)` 作为内部顺序，避免真实 EP0 后的各集偏移一位；面板按钮仍显示 Bangumi 的实际 `sort`，因此 UBW 这类条目显示为 `0, 01, …, 12`。
- 官方番剧在站内切换季度时，以当前 URL 和页面实时 `md/ss/分区` 标识为准，不复用上一季残留的初始化数据或通用标题绑定；检测到旧绑定与当前季度冲突时会重新显示绑定入口。
- 支持 OP/ED 一键跳过：设置面板配置全局默认秒数；鼠标悬停播放器「跳OP/ED」按钮会弹出滑条（20–100 秒，5 秒步进），拖动即为当前番剧保存专属时长并优先于全局默认。按钮显示开关与快捷键仍按番剧/全局保存。
- 支持弹幕悬停 `+1`、本地弹幕收藏，并复用 Bilibili 当前弹幕发送设置。
- 可选显示角色 / CV 横栏、Bangumi 风格条目信息栏、Bilibili 白名单和轻量候选提示。
- 设置中的「官方 API 连接失败后自动使用 api.bgmapi.com」默认关闭。启用后仍先探测官方 api.bgm.tv，但探测超时缩短为约 2 秒且不额外重试；连接失败或超时后自动回退并跳过逐次确认弹窗。
- 浏览器插件版会在面板处于活动状态或打开设置时自动检查 GitHub 上的 `extension/manifest.json`，失败后改查 GitCode 的提交固定版本；发现新版会显示下载入口。已解压扩展仍需覆盖原目录并在扩展管理页点击“重新加载”，不会下载或执行远程代码。

## 已知限制

- 需要自行提供 Bangumi Access Token；多数番剧第一次使用仍需要手动确认绑定。
<span style="color:#bd2441"><strong>危险：</strong>如果在官方 API 无法连接时手动选择 <code>api.bgmapi.com</code>，或启用自动回退开关，Bangumi Access Token、<code>Authorization</code> 请求头、收藏/评分/进度等请求内容会经过第三方服务器。第三方可能读取或记录这些信息；它不是 Bangumi 官方服务。仅在信任该站点时使用，并在怀疑泄露后立即吊销 Token。</span>
...
- 当前插件版按“加载已解压的扩展程序”分发，浏览器不会替它自动覆盖本地文件；内置更新功能只负责版本检测和打开下载页。

## 测试

[测试用例与运行方法](./tests/README.md)列出本次修复的触发条件、预期结果、自动化回归文件，以及需要真实浏览器验证的场景。GitHub Actions 会执行 `tests/*.test.js` 中的全部测试。
