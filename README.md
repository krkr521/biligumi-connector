...
- 默认启用实验性的长视频分集推测入口：绑定时若当前视频超过 2 小时，面板会询问它是否在一个视频内包含连续多集正片；选择“是”后，才会按 Bangumi 单集时长和当前 UP 专属的“首集开始时间”估算当前集，并使用单集局部进度自动标记看过。
...
- 长视频分集推测入口默认开启，但每个超过 2 小时的视频仍需在绑定时人工确认；选择“否”会按普通视频处理。它假设正片各集连续排列。若 UP 在集间插入额外片段、Bangumi 时长资料不准确或首集起点未校准，推测结果可能偏移。只要 Bangumi 存在缺失的单集时长，或累计时间线比视频总长多出超过 5 分钟，就只显示推测，不自动更新进度。
