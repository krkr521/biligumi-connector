...
- 普通投稿的多季度合集若使用 `1.1 / 1.2`、`第1集上 / 下`、`第二季0`、`S2E13` 等分 P 标题，会按“季度 + 集数范围”绑定不同 Bangumi 条目。首次在每个范围绑定时会根据 Bangumi 正片话数给出范围确认；未映射或冲突的分 P 不会回退到整 BV 的旧绑定。每集拆成上下段时，必须两段都达到观看阈值才自动标记该集。
...
- 支持 OP/ED 一键跳过：设置面板配置全局默认秒数；鼠标悬停播放器「跳OP/ED」按钮会弹出滑条（20–100 秒，5 秒步进），拖动即为当前番剧保存专属时长并优先于全局默认，也可一键恢复跟随全局。按钮显示开关与快捷键仍按番剧/全局保存。
