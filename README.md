...
- Bangumi 风格条目信息栏会解析 Bangumi 页面以补全制作人员链接，对界面排版改动较大，也会带来一定性能开销；默认关闭，可在设置里手动开启。
- 官方番剧源的右侧栏兼容属于实验功能，不推荐作为主要同步入口；齿轮设置里可以关闭“官方番剧页右侧布局兼容”。
- 白名单支持 UP 主 UID/名称、BV 号、页面 key 或 URL 片段；齿轮按钮会同时配置 Access Token、白名单、轻量候选开关、角色 / CV 横栏、条目信息栏、官方番剧页布局兼容开关和自动标记已看的进度标准线。
