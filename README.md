...
- 长视频多集合一识别：普通视频超过 2 小时会询问是否在一个视频内包含连续多集正片；选择“是”后，才会按 Bangumi 单集时长和当前 UP 专属的“首集开始时间”估算当前集，并使用单集局部进度自动标记看过。设置中可勾选「遇到长视频时自动识别」（默认关闭）：勾选后跳过确认，直接按多集合一处理。Bangumi 动画条目的 `platform` 为「剧场版」或 `Movie` 时按动画电影处理；平台字段缺失或无法识别时，再以“正片恰好 1 集且该集超过 1 小时”兜底。动画电影不会触发长视频集数推断。
