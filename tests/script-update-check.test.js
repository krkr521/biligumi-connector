...
assert.equal(constants.SCRIPT_VERSION, "0.7.19");
...
cacheSandbox.cached = { remoteVersion: "0.7.20", sourceId: "github", url: githubSource.url, checkedAt: cacheNow - 1000 };
assert.equal(cacheSandbox.updateCache.readCachedScriptUpdateState().status, "available");
cacheSandbox.cached = { remoteVersion: "0.7.20", sourceId: "github", url: "https://evil.example/update.user.js", checkedAt: cacheNow - 1000 };
assert.equal(cacheSandbox.updateCache.readCachedScriptUpdateState().status, "idle");
cacheSandbox.cached = { remoteVersion: "0.7.20", sourceId: "github", url: githubSource.url, checkedAt: cacheNow + 1000 };
assert.equal(cacheSandbox.updateCache.readCachedScriptUpdateState().status, "idle");
cacheSandbox.cached = { remoteVersion: "0.7.20", sourceId: "github", url: githubSource.url, checkedAt: cacheNow - constants.SCRIPT_UPDATE_CACHE_TTL_MS - 1 };
assert.equal(cacheSandbox.updateCache.readCachedScriptUpdateState().status, "idle");
